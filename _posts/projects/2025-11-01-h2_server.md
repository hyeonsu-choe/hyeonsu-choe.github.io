...
last_modified_at: 2025-11-23 21:05 +0900
...
acceptor에서 worker로의 작업 요청은 load balancer에 의해 분산 되어 전달 된다.  
...
- accept된 클라이언트 소켓을 **LoadBalancer**를 이용하여 각 Worker 큐에 분배  
...
로드 밸런서는 입력된 요청을 받아 해시 연산을 통해 처리 할 Worker Queue의 번호를 얻는다.   
그리고 Worker Queue가 정상인 상태라면 그대로 요청을 전달한다.   
그러나 만약 선택된 Worker Queue 가 full인 경우, 다른 여유가 있는 Worker Queue를 `Round Robin` 스케쥴링을 통해 찾아 작업을 요청한다.  

이때, 선택된 Worker Queue의 번호는 hash 연산을 통해 접근된 indirect_table의 entry에 갱신 되어 이후 이 entry에 접근하는 요청은 새롭게 선택된 Worker Queue로 전달 되게 된다.  

만약, `Round Robin` 스케쥴링을 통해 Worker Queue 들을 한 바퀴 순회 하였음에도 여유가 있는 Worker Queue를 발견하지 못하는 경우에는 해당 요청은 버려진다.   
...
💡 **참고:** 본 절의 내용과 수치는 **Load Balancer 도입 이전 버전**을 기준으로 한다.  
...
## 결론
두 구조는 성능적으로 유사하지만, 본 프로젝트에서는 다음과 같은 이유로 **Acceptor/Worker 구조**를 택함.
